...
class Preprocessor:
    ...
    def impute_categorical(self): # impute missing values for categorical features with the most frequent value
        for col in self.categorical_cols:
            if self.df[col].dtype == 'object':  
                self.df[col] = self.df[col].map({'Yes': 1, 'No': 0,'Male': 1, 'Female': 0,'Infected': 1, 'Non-infected': 0})
    ...
    def calculate_bins(self,col,bins):
        min_val=min(col)
        max_val=max(col)
        bin_width=(max_val-min_val)/bins
        return [min_val + i * bin_width for i in range(bins + 1)]
    
    def bin_column(self,col_data,bin_edges):
        binned_data=[]
    ...
    def bin_numeric(self):
        for col in self.numeric_cols:
            ...
            bin_edges=self.calculate_bins(col_data,9)
            self.df[col]=self.bin_column(col_data,bin_edges)
